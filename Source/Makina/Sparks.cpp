...
Sparks::Sparks(D3DAppValues *values, ID3D11ShaderResourceView* randomTexSRV)
: ParticleSystem(values, randomTexSRV, 3000)
{
	mParticleUpdate = mValues->mParticlesFX->GetTechniqueByName("SparksUpdate");
	mParticleDraw = mValues->mParticlesFX->GetTechniqueByName("SparksDraw");
...
void Sparks::BuildVertexLayout()
{
	...
	if(FAILED(hr))
		throw UnexpectedError(L"Failed to create input layout in SparksSystem!");
}