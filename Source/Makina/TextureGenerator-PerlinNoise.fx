...
#ifndef PERLINNOISE_FX
#define PERLINNOISE_FX

#include "TextureGenerator-Utilities.fx"

[numthreads(THREADS_NUMBER, 1, 1)]
void CSPerlin(int3 groupThreadID : SV_GroupThreadID, int3 dispatchThreadID : SV_DispatchThreadID)
{
	float tex = 0;
	...
	for (uint i = 0; i < gOctaves; i++)
	{
		...
		tex += pow(abs(1/gLacunarity), i) * sampledColor;
	}

	tex = ScalePerlin(tex, 0, 1);
	
	gOutput[dispatchThreadID.xy] = float4(tex, tex, tex, 1.0f);
}

[numthreads(THREADS_NUMBER, 1, 1)]
void CSSphericalPerlin(int3 groupThreadID : SV_GroupThreadID, int3 dispatchThreadID : SV_DispatchThreadID)
{
